...
class DishCategoriesControllerTest < ActionController::TestCase
	...
	test 'should redirect create when not logged in' do
		assert_no_difference 'DishCategory.count' do
			post :create, dish_category: { name: "new-cate", description: "new category", image: File.open(File.join(Rails.root, '/test/fixtures/images/salad.jpg')) }
		end	
	...
	test 'should redirect update when not logged in' do
		patch :update, id: @dish_cate, dish_category: { name: "edited cate", description: "edited category", image: File.open(File.join(Rails.root, '/test/fixtures/images/salad2.jpg')) }
		assert_redirected_to login_url
		assert_not flash.empty?
	end
